...
public class BoardUtils {
    ...
    public static final boolean[] EGHTH_RANK = initRow(53);
    ...
    private static boolean[] initRow(int indexTile) {
        final boolean[] row = new boolean[NUM_TILES];

        do{
            row[indexTile] = true;
            indexTile++;
        }while(indexTile % NUM_TILES_PER_ROW != 0);

        return row;
    }

    public static boolean isTileValidCoordonate(final int candidateDestinationCoordonate) {
        return candidateDestinationCoordonate>=0 && candidateDestinationCoordonate<NUM_TILES;
    }
